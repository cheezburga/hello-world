...
//			double[] trainedTheta = new double[result.getSecond().length];
//			for(int i=0; i<result.getSecond().length; i++){
//				trainedTheta[i] = result.getSecond()[i];
//			}
//			INDArray theta = Nd4j.create(trainedTheta).transpose();
//			result = costFunction(theta, X, y);
//				
//			System.out.println("Cost after training theta for "+iter +" times "+result.getFirst()+"\n");
//			System.out.println("Gradient after training theta for "+iter +" times "+Arrays.toString(result.getSecond())+"\n");
//			
//		}
		
		//============= Part 3: Optimizing using fminunc  =============
		double[] thetaFromFminunc = new double[]{-25.161272,0.206233,0.201470};//TODO dsze need to find fminunc java implementation instead of cheating off of octave
		
		
		
		
		INDArray plotX = Nd4j.vstack(x.getColumn(0).min(0).sub(2), x.getColumn(0).max(0).add(2));//use 2 to offset in case they have the same point
		INDArray plotY = plotX.mul(thetaFromFminunc[1]).add(thetaFromFminunc[0]).mul(-1.0/thetaFromFminunc[2]);//(-1./theta(3)).*(theta(2).*plot_x + theta(1));
