package com.sli.neural_networks;
...
public class Ex4 {
	...
	public static void main(String[] args) throws Exception {
		...
		INDArray x_5000x400 = load("machine-learning/neural_network/ex4data1.txt", totalSamples, " ").getColumns(colIndices);
...
//		}
		...
		INDArray y_5000x1 = load("machine-learning/multiclass_classification/ex4data2.txt", totalSamples, " ");
		...
		INDArray theta1_25x401 = load("machine-learning/multiclass_classification/ex4weights1.txt", 25, " ").getColumns(colIndices);//do not know why it has 402 columnes
		...
		INDArray theta2_10x26 = load("machine-learning/multiclass_classification/ex4weights2.txt", 10, " ").getColumns(colIndices);//do not know why it has 27 columnes;
		INDArray nn_params = Nd4j.hstack(theta1_25x401.ravel(), theta2_10x26.ravel());//unroll paramters
...
//	    System.out.println(Arrays.toString(arr));
	...
	//int hidden_layer_size = 25;
	//int num_labels = 10;
	private static void nnCostFunction(INDArray nn_params, int input_layer_size, int hidden_layer_size, int num_labels, 
			INDArray x, INDArray y, double lambda){
		int theta1Length = hidden_layer_size*(input_layer_size+1); 
		INDArray theta1 = nn_params.get(NDArrayIndex.interval(0, theta1Length)).reshape(hidden_layer_size, input_layer_size+1);//25x401
		int theta2Length = num_labels*(hidden_layer_size+1); 
		INDArray theta2 = nn_params.get(NDArrayIndex.interval(theta1Length, theta1Length+theta2Length)).reshape(num_labels, hidden_layer_size+1);//10x26
		int m = x.rows();
		INDArray X = Nd4j.hstack(Nd4j.ones(m, 1), x);
		INDArray activationAtHiddenLayer = Transforms.sigmoid(X.mmul(theta1.transpose()));
		INDArray activationAtOutputLayer = Transforms.sigmoid(Nd4j.hstack(Nd4j.ones(activationAtHiddenLayer.rows(), 1),activationAtHiddenLayer).mmul(theta2.transpose()));
